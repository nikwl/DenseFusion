...
ROOT="/home/nikolas/dev/data4/DenseFusion"
if [ ! -d "$ROOT"/datasets/ycb/YCB_Video_Dataset ];then
...
&& mv YCB_Video_Dataset/ "$ROOT"/datasets/ycb/ \
&& rm YCB_Video_Dataset.zip
fi

if [ ! -d "$ROOT"/datasets/linemod/Linemod_preprocessed ];then
...
&& mv Linemod_preprocessed/ "$ROOT"/datasets/linemod/ \

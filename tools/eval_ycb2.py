...
selected_objects = opt.object_idx

for now in range(startframe, 2949):
        ...
        try:
            ...
            if itemid == selected_objects:
                ...
                json.dump(
                    tf,
                    open(
                        '{0}/{1}-tf.json'.format(opt.dataset_root, testlist[now]),
                        "w",
                    )
                )
                print("Saved tf for frame ", testlist[now])
